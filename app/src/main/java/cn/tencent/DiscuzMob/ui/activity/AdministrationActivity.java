...
/*管理工作*/
public class AdministrationActivity extends BaseActivity implements SwipeRefreshLayout.OnRefreshListener, ILoadListener {
    ...
    private void initView() {
        ...
        adapter = new MythreadsAdapter(AdministrationActivity.this);
...
//                        Log.e("TAG", "获取公参 =" + response.body().string());
    ...
    private void load(final boolean append) {
        RedNet.mHttpClient.newCall(new Request.Builder()
                .addHeader("Cookie", cookiepre_auth + ";" + cookiepre_saltkey)
                .url(AppNetConfig.ADMINISTRATION + "&page=" + String.valueOf(append ? ++mPage : (mPage = 1)))
                .cacheControl(new CacheControl.Builder().noStore().noCache().build()).build())
                .enqueue(new Callback() {
                    @Override
                    public void onFailure(Request request, IOException e) {
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                Toast.makeText(AdministrationActivity.this, "网络请求失败", Toast.LENGTH_SHORT).show();
                            }
                        });
                    }
