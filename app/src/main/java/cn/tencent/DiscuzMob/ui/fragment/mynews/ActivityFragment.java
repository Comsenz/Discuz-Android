package cn.tencent.DiscuzMob.ui.fragment.mynews;
...
import cn.tencent.DiscuzMob.ui.activity.MyThreadActivity;
...
public class ActivityFragment extends SimpleRefreshFragment implements SwipeRefreshLayout.OnRefreshListener, ILoadListener {

    private String cookiepre_auth;
    private String cookiepre_saltkey;
    private FooterForList mFooter;
    private MythreadsAdapter adapter;
    private Activity activity;
    private int mPage;

    @SuppressLint("ValidFragment")
    public ActivityFragment(MyThreadActivity myThreadActivity) {
        this.activity = myThreadActivity;
    }
...
//                        Log.e("TAG", "获取公参 =" + response.body().string());
    ...
    private void load(final boolean append) {
        RedNet.mHttpClient.newCall(new Request.Builder()
                .addHeader("Cookie", cookiepre_auth + ";" + cookiepre_saltkey)
                .url(AppNetConfig.MYACTVITY + "&page=" + String.valueOf(append ? ++mPage : (mPage = 1)))
                .cacheControl(new CacheControl.Builder().noStore().noCache().build()).build())
                .enqueue(new Callback() {
                    @Override
                    public void onFailure(Request request, IOException e) {
